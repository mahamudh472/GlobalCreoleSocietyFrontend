  ...
  return useMutation({
    mutationFn: async ({ societyId, societyData }) => {
      const response = await apiMethods.patch(
        ENDPOINTS.SOCIETIES.UPDATE(societyId),
        societyData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
    ...
    onMutate: async ({ societyId, societyData }) => {
      ...
      const previousSociety = queryClient.getQueryData(
...
export const useJoinSocietyMutation = () => {
  ...
  return useMutation({
    mutationFn: async (societyId) => {
      const response = await apiMethods.post(
        ENDPOINTS.SOCIETIES.JOIN(societyId)
      );
  ...
  return useMutation({
    mutationFn: async (societyId) => {
      const response = await apiMethods.delete(
        ENDPOINTS.SOCIETIES.LEAVE(societyId)
      );
      return { societyId, data: response.data };
    },
    onMutate: async (societyId) => {
...
export const useRemoveMemberMutation = () => {
  ...
  return useMutation({
    mutationFn: async ({ societyId, memberId }) => {
      ...
      const response = await apiMethods.post(
        `${ENDPOINTS.SOCIETIES.DETAIL(societyId)}/remove-member/`,
        {
          member_id: memberId,
        }
      );
