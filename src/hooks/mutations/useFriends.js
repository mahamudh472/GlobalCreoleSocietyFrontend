  ...
  return useMutation({
    mutationFn: async ({ userId, action }) => {
      ...
      const response = await apiMethods.post(
        ENDPOINTS.FRIENDS.RESPOND_REQUEST(userId),
        {
          action,
        }
      );
      ...
      if (previousRequests) {
        queryClient.setQueryData(
          ...
          previousRequests.filter(
            (request) => request.id !== userId && request.sender?.id !== userId
          )
        );
      }
    ...
    onError: (error, { userId, action }, context) => {
      ...
      toast.error(
        error.response?.data?.error || "Failed to respond to request"
      );
    },
  });
};
...
export const useBlockUserMutation = () => {
  ...
  return useMutation({
    ...
    onSuccess: ({ userId }) => {
      ...
      queryClient.invalidateQueries({ queryKey: queryKeys.friends.requests() });
      // Also invalidate blocked users list so UI updates immediately
      queryClient.invalidateQueries({
...
export const useUnblockUserMutation = () => {
  ...
  return useMutation({
    ...
    onSuccess: ({ userId }) => {
      ...
      // Also invalidate blocked users list to remove the user from UI
      queryClient.invalidateQueries({
        queryKey: queryKeys.users.blockedUsers(),
      });
