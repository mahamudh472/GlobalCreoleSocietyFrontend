...
const ShareModal = ({ isOpen, onClose, postData, postId }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/15 flex items-center justify-center z-50 transition-opacity duration-300 p-2 sm:p-4">
      <div
        ref={popupRef}
        className="bg-white rounded-xl w-full sm:w-[85%] md:w-[65%] lg:w-[50%] max-h-[90vh] overflow-hidden transform transition-transform duration-500 ease-out"
      >
        {/* Header */}
        <div className="flex items-center justify-between p-3 sm:p-4 border-b border-gray-200">
          <h2 className="text-lg sm:text-2xl font-bold text-gray-900">Share</h2>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto max-h-[calc(90vh-120px)]">
          ...
          <div className="px-3 sm:px-4 py-3 border-b border-gray-100">
            <div className="flex items-center space-x-2 sm:space-x-3 mb-3">
              ...
              />
              <span className="font-bold text-gray-900 text-sm sm:text-base">{displayName}</span>
            </div>
            ...
            <textarea
              ...
              className="w-full p-2 sm:p-3 border border-gray-200 rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm sm:text-base"
              rows="3"
            />
            ...
            <div className="flex justify-end mt-3">
              <button
                ...
                className="px-3 sm:px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-xs sm:text-sm font-medium cursor-pointer"
              >
                Share now
              </button>
            </div>
          ...
          <div className="px-3 sm:px-4 py-3 sm:py-4">
            ...
            <div className="flex items-center space-x-2 sm:space-x-3 overflow-x-auto pb-2 gap-3 sm:gap-5">
              ...
              {Array.isArray(conversations) &&
                ...
                conversations.map((conv) => (
                  <div key={conv.id} className="flex-shrink-0 text-center">
                    ...
                    <span className="text-[10px] sm:text-xs text-gray-600 mt-1 block truncate max-w-[50px] sm:max-w-none">
                      {conv.name}
                    </span>
                  </div>
                ))}
              <button className="flex-shrink-0 w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-colors">
        ...
        <div className="p-3 sm:p-4 border-t border-gray-200 flex justify-end">
          <button
            onClick={handleShareNow}
            className="px-4 sm:px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-xs sm:text-sm font-medium cursor-pointer"
          >
            Share
          </button>
        </div>
